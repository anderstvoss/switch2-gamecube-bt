...
1. This branch hands off the first working-connection investigation to a native
   Linux machine. Follow `docs/LINUX-AGENT-HANDOFF.md`; do not use WSL1 or WSL2
   on the current host.
2. Begin with a bounded, read-only BlueZ discovery diagnostic. Request user
   SYNC only once the command is prebuilt and checked. Do not pair or connect
   before a sanitized candidate appears and the user confirms it.
3. The ESP32-S3 bridge remains an external hardware fallback. Do not buy,
   flash, configure, or connect one without explicit approval.
